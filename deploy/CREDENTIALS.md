...
|---|---|---|---|
| **Dashboard login** | `SROW_USERS` in `/etc/syncrow/portal.env` | root:640 | Comma-separated `user:password` pairs; the app's session login checks these. |
| **App config + Influx creds** | `/etc/syncrow/portal.env` | root:640 | `INFLUX_URL/TOKEN/ORG/ORG_ID/BUCKET`, `PORTAL_DOMAIN`, `COOKIE_SECRET`. Loaded by systemd. |
...
# edit SROW_USERS in /etc/syncrow/portal.env (user:pass,user2:pass2), then:
